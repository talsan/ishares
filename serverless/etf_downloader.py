...
def lambda_handler(event, context):
    print(f'event: {event}')
    try:
        print(f'processing {event["etf"]} data for {event["holdings_date"]}')
        main(ticker=event['etf'], holdings_date=event['holdings_date'])
        #print('pretend we just downloaded here')
        out_msg = {
            'statusCode': 200,
        ...
        output = f'{additional_information} -- error:{str(e)}'
        #raise Exception(f"Lambda function encountered an error: {output}")
